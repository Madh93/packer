...
- `parallels_tools_guest_path` (string) - The path in the virtual machine to
  ...
  template](/docs/legacy_json_templates/egine) that has a single
...
- `prlctl` (array of array of strings) - Custom `prlctl` commands to execute
  ...
  template](/docs/legacy_json_templates/egine), where the `Name`
...
`prlctl`. Each argument is treated as a [template engine](/docs/legacy_json_templates/egine). The only available
