...
[communicator](/docs/legacy_json_templates/communicators) can be configured for this
builder.
...
- `omi_name` (string) - The name of the resulting OMIS that will appear when managing OMIs in the Outscale console or via APIs. This must be unique. To help make this unique, use a function like `timestamp` (see [template engine](/docs/legacy_json_templates/egine) for more info).
...
- `omi_description` (string) - The description to set for the resulting OMI(s). By default this description is empty. This is a [template engine](/docs/legacy_json_templates/egine), see [Build template
...
- `tags` (object of key/value strings) - Tags applied to the OMIS and relevant snapshots. This is a [template engine](/docs/legacy_json_templates/egine), see [Build template data](#build-template-data) for more information.
